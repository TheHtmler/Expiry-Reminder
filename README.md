...
当前处于**首期 MVP 顺序实施中**的阶段。

- 产品与技术设计：已确认并提交
- 首期实施计划：共 14 个任务，Task 1 已提交，Task 2 至 Task 7 已实现并暂缓提交
- Task 8：首页、物品列表、手动录入、详情和回收站已完成代码与自动验证，等待
  CloudBase 数据库初始化后的开发环境走查
- 微信小程序账号：已创建
- 小程序名称：到期咯
- AppID：`wxb8bd2ab35c41a7cd`
- 默认分支：`main`
- 下一步：初始化 CloudBase 集合、索引和规则，完成 Task 8 开发环境全路径走查

## 核心功能

- 首次进入自动创建“我的家”，当前用户为管理员
- 一个用户可以创建或加入多个家庭
- 管理员与普通成员两级权限
- 家庭成员共享物品、数量、状态和提醒
- 条形码、二维码扫码录入
...
仓库依赖与构建脚本已经建立，使用 `npm install` 安装根目录依赖；`cloudfunctions/api`
使用独立依赖并已部署到当前 CloudBase 开发环境。
